				...
				if($newUser != ''){
					...
					socket.emit('new user',$newUser,function(data){
						if(data){
							$chatWrap.slideDown();
							$loginWrap.slideUp();
							$('#title').html('&nbsp &nbsp <img src = "cv.png"/> &nbsp&nbsp <img src = "g.gif"/> &nbsp &nbsp&nbsp &nbsp&nbsp &nbsp&nbsp &nbsp &nbsp&nbsp &nbsp&nbsp &nbsp&nbsp&nbsp &nbsp&nbsp &nbsp&nbsp &nbsp&nbsp&nbsp &nbsp&nbsp &nbsp&nbsp &nbsp&nbsp &nbsp&nbsp &nbsp&nbsp &nbsp  <img src  = "sp.png"/>  Welcome, '+$newUser  );
							$username.val('');
						}else{
...
// the webrtc start from here ten ten tenaaanaaaaannnnnn :-)
...
//callllllllllllllllllllllerr coddddeeeeeeeee
		...
		if(remoteUser != ''&&remoteUser != self){
			...
			socket.emit('newVideoChatRequest',{sender:self,receiver:remoteUser},function(data){
				...
				if(data.response){
					...
					$('#video-chat').children('h3').css('background-color','#2AA9D0');
